...
import { Globe, Star, Clock, Package, MessageSquare, ShoppingBag, Layers, FileText, ShieldCheck, CheckCircle2 } from 'lucide-react';

export default function ProductDetailSpecs({ product, isOwner }) {
  return (
    <div className="space-y-8">
      {/* 1. 수량별 구간 단가표 및 핵심 B2B 스펙 */}
        ...
        <div className="p-5 bg-slate-900 text-white rounded-2xl border border-slate-800 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center pt-1 border-t border-slate-800">
            {(product?.tiered_pricing || [
              ...
              <div key={idx} className="p-2.5 bg-slate-800/60 rounded-xl border border-slate-700/60 space-y-0.5">
      ...
      {/* 2. 알리바바 스펙 표 (Attribute Table) & 리치 본문 */}
